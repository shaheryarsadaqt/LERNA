...
class LERTracker:
    """
    Learning Efficiency Rate (LER) tracker with validation.
    
    LER = (Information Gain) / (Compute Cost)
    
    Information gain approximated as: loss reduction × entropy(logits)
    """
    ...
    def __init__(
        ...
        """
        Initialize LER tracker with validation capabilities.
        
        Args:
            task: Task name for calibration
            window_size: Window size for LER calculation
            validate_correlation: Whether to validate correlation with performance
            task_calibration: Task-specific calibration parameters
        """
        self.task = task
        self.window_size = window_size
        ...
        # Validation tracking
        self.correlation_history: List[Tuple[float, float]] = []  # (LER, accuracy)
    ...
    def update(
        ...
        """
        Update tracker with new training step.
        
        Args:
            loss: Current loss value
            logits: Model logits for entropy calculation
            accuracy: Current accuracy (for validation)
            n_steps: Number of steps since last update
        """
        ...
        # Compute LER if we have enough history
        if len(self.loss_history) >= 2:
            # Loss reduction
            loss_gain = abs(self.loss_history[-2] - self.loss_history[-1])
            
            # Average entropy in window
...
class EfficiencyMetricsCollector:
    ...
    def update(
        ...
        # Update LER tracker
        self.ler_tracker.update(loss, logits, accuracy)
