...
Generates a Cartesian matrix of baselines x GLUE tasks x seeds.
...
def validate_results_json(path: Path, entry: dict) -> tuple:
    try:
    ...
    if data.get("early_stopping_enabled") != es_enabled:
        return False, f"mismatch_early_stopping_enabled: {data.get('early_stopping_enabled')!r} != {es_enabled}"
    if data.get("early_stopping_patience") != es_patience:
        return False, f"mismatch_early_stopping_patience: {data.get('early_stopping_patience')!r} != {es_patience}"
    return True, "ok"
...
def execute(args):
    ...
    if profile == "server":
        ...
        if device.type != "cuda":
            raise RuntimeError(f"Server profile requires CUDA, got {device}")
    ...
    manifest = {
        ...
        "dataset_scope": "full GLUE" if max_samples is None else "sample-capped",
        "wandb": "disabled",
        "entries": entries,
    }
    manifest_path.write_text(json.dumps(manifest, indent=2, default=str))
